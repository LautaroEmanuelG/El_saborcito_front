...
const ScreenCategoriaPadreArticulo = () => {
  ...
  // Filtrar: solo categorías padre y excluir "insumo"/"insumos"
  const categoriasPadreFiltradas = categoriasPadre.filter(
    (cat: Categoria) =>
      !cat.tipoCategoria && !['insumo', 'insumos'].includes(cat.denominacion.trim().toLowerCase())
  );
  ...
  const handleSubmit = async (values: Partial<Categoria>) => {
    if (modalMode === 'add') {
      await addCategoriaPadre({ ...values, tipoCategoria: null });
    } else if (modalMode === 'edit' && selectedCategoria?.id) {
      await updateCategoriaPadre({ ...values, id: selectedCategoria.id, tipoCategoria: null });
    }
    setOpenModal(false);
  };
