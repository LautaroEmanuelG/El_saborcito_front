...
const API_BASE_URL = '/forma-pagos';

export const saveFormaPago = async (data: any) => {
...
export const getFormaPagoById = async (id: number) => {
  const response = await axiosInstance.get(`${API_BASE_URL}/${id}`);
  return response.data;
};

export const getAllFormaPagos = async () => {
