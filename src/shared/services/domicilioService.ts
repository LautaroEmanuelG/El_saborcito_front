...
const API_BASE_URL = '/domicilios';

export const saveDomicilio = async (data: any) => {
  const response = await axiosInstance.post(`${API_BASE_URL}/${data.id}`, data);
  return response.data;
};

export const deleteDomicilio = async (id: number) => {
  const response = await axiosInstance.delete(`${API_BASE_URL}/${id}`);
  return response.data;
};

export const getDomicilioById = async (id: number) => {
